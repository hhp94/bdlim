...
arma::vec dbern_cpp(const arma::vec& y, const arma::vec& linear_pred, arma::vec& probs) {
  probs = 1.0 / (1.0 + arma::exp(-linear_pred));
  return y % arma::log(probs) + (1 - y) % arma::log(1 - probs);
}
...
Rcpp::List bdlim1_logistic_cpp(const arma::vec& y,
  ...
  // weighted design for pg
  arma::mat design_w(design.n_rows, design.n_cols);
  int n = y.n_elem;

  // init pg
  arma::vec w_pg(n);
  std::vector<double> h_vec(n, 1.0);  // h is fixed at 1.0
  arma::vec pred_mean_model_scale(n, arma::fill::zeros);
  ...
  // needed for dbern_cpp. Create a vector of probs and create subsets by group
  arma::vec probs(n);

  // create subsets of probs for each group so that we can use subset by pointer.
  // values are ALWAYS calculated before updated.
  std::vector<arma::vec> probs_subsets(n_weight_groups);
  for (uint16_t j = 0; j < n_weight_groups; ++j) {
    probs_subsets[j] = probs.elem(*g_idx_ptrs[j]);
  }
  ...
  for (uint32_t i = 0; i < nits; ++i) {
    rpg_hybrid(w_pg.memptr(), h_vec.data(), pred_mean_model_scale.memptr(), &n);
    design_w = design.each_col() / (1.0 / arma::sqrt(w_pg));
    V = arma::symmatu(design_w.t() * design_w);
    V.diag() += diag_update;
    V = arma::inv_sympd(V);
    m = V * (design.t() * (y - .5));
    ...
    for (uint16_t j = 0; j < n_weight_groups; ++j) {
      ll = arma::sum(
        dbern_cpp(
          y.elem(*g_idx_ptrs[j]),
          design.rows(*g_idx_ptrs[j]) * regcoef,
          probs_subsets[j]
        )
      );
    ...
    ll_all_keep.col(i) = dbern_cpp(y, pred_mean_model_scale, probs);
  }
