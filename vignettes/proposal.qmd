...
## `bdlim4`
* Allow specifying which pattern to fit in `bdlim4` using the `model` argument. Setting `model = "all"` will fit all four patterns.
...
* Handling posterior draws of multiple chains can become very cumbersome. I propose to use the [`{posterior}`](https://mc-stan.org/posterior/index.html) package to do so in a consistent way. These objects are named `draws` and stored under each `bdlim1` fit.
```{r}
# get the draws object of `fit_n`
f_mcmc$fit_w$draws |> class()
```
...
```{r}
merge_chains(
  subset_draws(f_mcmc$fit_w$draws, iteration = 101:200, variable = ".W_F_1")
)
```

* This also enables us to use many other Bayesian packages that support `{posterior}`. For example, we can use the package [`{bayesplot}`](https://mc-stan.org/bayesplot/articles/plotting-mcmc-draws.html) to trace plot our `sigma` draws.
```{r}
library(bayesplot)
color_scheme_set("viridis")
mcmc_trace(subset_draws(f_mcmc$fit_w$draws, variable = c("sigma"))) +
  panel_bg(fill = "#f1f3f5", color = NA)
```

Or plotting the cumulative effects. See more cool plots [`here`](https://mc-stan.org/bayesplot/reference/MCMC-traces.html): 
```{r}
mcmc_rank_overlay(subset_draws(f_mcmc$fit_w$draws, variable = c(".ce_F", ".ce_M"))) +
  panel_bg(fill = "#f1f3f5", color = NA)
```
...
## `bdlim1`
* Fixed a bug for `bdlim1` random effect. When random effect is true, selecting the column by `1:n_groups` will select the random effects instead of selecting the intercept of the groups in the design matrix.
...
* This major refactor makes the code easier to maintain due to reduced repetition. Additionally, it is now easier to implement `bdlim1_gaussian`, for example, in C++ instead of R to enhance performance using something like `{Rcpp}` and `{RcppArmadillo}`. See below for an experimental implementation in `{RcppArmadillo}`
...
* `makebasis` was changed to return the fitted basis of the data as well as the full svd object. This change is needed to implement a `predict` method in the future.
* Added `loglik_all` to `bdlim4`. Set to `TRUE` if the full loglik matrix is needed. 
* Added robust input validations. See details here [`R/bdlim4.R`](https://github.com/hhp94/bdlim/blob/debug_bdlim1/R/bdlim4.R#L153).
* Added unit testings with [`{testthat}`](https://testthat.r-lib.org/).

# `{RcppArmadillo}`
Given the refractor, I've implemented bdlim1 in `{RcppArmadillo}`.

## Gaussian
```{r}
plot_dlfun <- function(r_draws, cpp_draws, iters) {
  r_fit_dlfun <- as_draws_df(
    subset_draws(r_draws, variable = ".dl", regex = TRUE, iteration = iters)
  ) |> 
    tidyr::pivot_longer(cols = contains(".dl"))
  r_fit_dlfun$fit <- "R"
  
  cpp_fit_dlfun <- as_draws_df(
    subset_draws(cpp_draws, variable = ".dl", regex = TRUE, iteration = iters)
  ) |> 
    tidyr::pivot_longer(cols = contains(".dl"))
...
For Gaussian, using `{Rcpp}` we get the exact results as the R fit
```{r}
g_r <- gaussian_wrap(seed = 1234, fit_cpp = FALSE, nits = 200)
g_cpp <- gaussian_wrap(seed = 1234, fit_cpp = TRUE, nits = 200)

plot_dlfun(g_r$fit_n$draws, g_cpp$fit_n$draws, iters = 101:200) |> 
  ggplot(aes(y = value, x = fit, color = fit, fill = fit)) +
...
```{r}
bench::mark(
  gaussian_wrap(seed = 1234, fit_cpp = FALSE, nits = 200),
  gaussian_wrap(seed = 1234, fit_cpp = TRUE, nits = 200),
  check = F
)
```
...
For logistic, using `{Rcpp}` get similar results but not exact same.
```{r}
l_r <- logistic_wrap(seed = 1234, fit_cpp = FALSE, nits = 1000)
l_cpp <- logistic_wrap(seed = 1234, fit_cpp = TRUE, nits = 1000)

plot_dlfun(l_r$fit_n$draws, l_cpp$fit_n$draws, iters = 501:1000) |> 
  ggplot(aes(y = value, x = fit, color = fit, fill = fit)) +
  facet_wrap(~name, scales = "free_y") +
  stat_slab(alpha = 0.75) +
  stat_pointinterval(position = position_dodge(width = .2, preserve = "single")) +
...
```{r}
bench::mark(
  logistic_wrap(seed = 1234, fit_cpp = FALSE, nits = 200),
  logistic_wrap(seed = 1234, fit_cpp = TRUE, nits = 200),
  check = F
)
```
...
Identical results
```{r}
all(fit_bw_draws_0.4 == as.matrix(fit_bw_draws_new)[, 1:ncol(fit_bw_draws_0.4)])
```
...
fit_sex_new_binom_bw <- subset_draws(
  ...
  variable = "^.W_(F|M)_", # grab just the w
  regex = TRUE
) |>
  as_draws_df()

all(fit_sex_0.4_binom_bw == as.matrix(fit_sex_new_binom_bw)[, 1:ncol(fit_sex_0.4_binom_bw)])
```
